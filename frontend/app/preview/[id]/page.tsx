...
export default function PreviewPage() {
  ...
  const processUpload = async () => {
    try {
      setLoading(true)
      console.log(`🚀 [ShiftSync] Starter prosessering med metode: ${method.toUpperCase()}`)
      console.log(`📋 [ShiftSync] Upload ID: ${uploadId}`)
      
      const startTime = Date.now()
      const response = await apiClient.process(uploadId, method)
      const duration = Date.now() - startTime
      
      console.log(`✅ [ShiftSync] Prosessering fullført på ${duration}ms`)
      console.log(`📊 [ShiftSync] Funnet ${response.shifts.length} vakter med ${Math.round(response.confidence * 100)}% sikkerhet`)
      console.log(`🔧 [ShiftSync] Backend prosesseringstid: ${response.processing_time_ms}ms`)
      
      if (response.warnings.length > 0) {
        console.warn('⚠️ [ShiftSync] Advarsler:', response.warnings)
      }
      ...
      console.error('❌ [ShiftSync] Prosesseringsfeil:', err)
      if (err instanceof ApiError) {
        ...
        console.error('❌ [ShiftSync] Ukjent feil:', err)
